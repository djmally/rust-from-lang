# Error Handling
 ...
 fn bind_socket() -> Result<(), Error> {
     ...
     let maybe_socket3: Result<TcpStream> = TcpStream::connect("127.0.0.1:8000");
 ...
 ```rust
 // TODO make sure this compiles
 let val1 = err_fn1()?.err_fn2()?.err_fn3()?.err_fn4()?;
 let val2 = try!(try!(try!(try!(err_fn1()).err_fn2()).err_fn3()).err_fn4());
 ```
